...
export const RadioPlayer = () => {
    ...
    try {
      if (!audioRef.current.paused) {
        ...
        audioRef.current.play().catch(() => {
          // Ignore play errors as they're usually due to user interaction requirements
          setIsPlaying(false);
        });
      }
    } catch (error) {
      // Only log the error, don't show to user unless necessary
  ...
  useEffect(() => {
    ...
    audio.onerror = (e) => {
      ...
      // Only show error for network-related issues
    ...
    // Start playing immediately but handle any autoplay restrictions gracefully
    audio.play().catch((error) => {
      if (error.name === 'NotAllowedError') {
        // Autoplay was prevented, don't show error
        setIsPlaying(false);
      }
    });
        ...
        <div className="lg:col-span-2 flex flex-col justify-between gap-4">
          <div className="text-center sm:text-left">
            <img 
              src="/Logo darkk.png" 
              alt="Radio Logo" 
              className="h-8 sm:h-10 mb-2 inline-block"
            />
